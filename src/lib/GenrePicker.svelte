...
<div class="relative">
	{#if selected.length > 0}
		<div class="flex flex-wrap gap-1.5 mb-2">
			{#each selected as genre}
				<span class="flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-medium bg-sky-50 dark:bg-sky-950/50" style="color:#00B0F0">
					{genre}
					<button type="button" onclick={() => remove(genre)}
						class="leading-none opacity-60 hover:opacity-100 text-sm"
						aria-label="Usuń gatunek {genre}">×</button>
				</span>
			{/each}
		</div>
	{/if}
	...
	{#if open && (filtered.length > 0 || showAddNew)}
		<div class="absolute z-30 left-0 right-0 mt-1 bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-100 dark:border-gray-800 overflow-hidden max-h-48 overflow-y-auto">
			{#each filtered as genre}
				<button type="button" onmousedown={() => add(genre)}
			...
			{#if showAddNew}
				<button type="button" onmousedown={() => add(query)}
					class="w-full text-left px-4 py-2.5 text-sm border-t border-gray-50 dark:border-gray-800 hover:bg-sky-50 dark:hover:bg-sky-950/30"
					style="color:#00B0F0">
					Dodaj „{query.trim()}"
				</button>
			{/if}
	...
	<p class="text-[11px] text-gray-300 dark:text-gray-600 mt-1">Wybierz z listy lub wpisz nowy gatunek</p>
</div>
