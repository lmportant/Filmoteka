...
{#if $authLoading}
	<div class="min-h-screen bg-white dark:bg-gray-950 flex items-center justify-center">
...
{#if $user && showInstallBanner}
	<div class="fixed bottom-4 left-4 right-4 z-50 flex items-center justify-between gap-3
		...
		<div class="flex items-center gap-2 shrink-0">
			<button onclick={handleInstall} class="text-xs font-medium px-3 py-1.5 rounded-lg" style="background:#00B0F0">
				Zainstaluj
			</button>
			<button onclick={() => showInstallBanner = false} class="text-gray-400 hover:text-white p-1">
...
{#if showIOSModal}
	<div class="fixed inset-0 z-50 flex items-end justify-center bg-black/40 px-4 pb-6"
		...
		<div class="w-full max-w-sm bg-white dark:bg-gray-900 rounded-3xl p-6 space-y-4"
			...
			<h2 class="text-base font-semibold text-gray-900 dark:text-gray-100">Dodaj do ekranu głównego</h2>
			<ol class="space-y-3 text-sm text-gray-600 dark:text-gray-400">
				...
				<li class="flex items-start gap-3">
					...
					<span>Przewiń i stuknij <strong>Dodaj do ekranu głównego</strong></span>
				</li>
				<li class="flex items-start gap-3">
			...
			<button onclick={() => { showIOSModal = false; showInstallBanner = false; }}
				class="w-full py-3 rounded-2xl text-sm font-medium text-white" style="background:#00B0F0">
				Rozumiem
			</button>
		</div>
	</div>
...
{#if $needRefresh}
	<div class="fixed bottom-4 left-4 right-4 z-50 flex items-center justify-between gap-3
		...
		<button onclick={() => updateServiceWorker(true)} class="text-xs font-medium px-3 py-1.5 rounded-lg" style="background:#00B0F0">
			Odśwież
		</button>
	</div>
{/if}
