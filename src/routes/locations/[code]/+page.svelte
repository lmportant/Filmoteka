...
<div class="min-h-screen bg-white">
	<header class="sticky top-0 z-10 bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
		<a href="/locations" class="text-gray-400 hover:text-gray-600 p-1 -ml-1" aria-label="Wróć">
			<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
				<path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
			</svg>
		</a>
		<div>
			<h1 class="text-base font-semibold text-gray-900 leading-tight">{code}</h1>
			{#if !loading}
				<p class="text-[11px] text-gray-400">{total.toLocaleString('pl-PL')} filmów</p>
			{/if}
		</div>
	</header>
	...
	{:else if movies.length === 0}
		<div class="px-4 pt-16 text-center">
			<p class="text-gray-300 text-sm">Brak filmów w tej lokalizacji.</p>
		</div>

	{:else}
		<div class="px-4 pt-2 pb-8 divide-y divide-gray-50">
				...
				<a href="/movie/{m.id}" class="py-2.5 flex items-center justify-between gap-3 hover:bg-gray-50/50 -mx-1 px-1 rounded-lg transition-colors">
					<div class="min-w-0 flex-1">
						...
						{#if m.tmdb_title_original && m.tmdb_title_original !== m.title}
							<p class="text-[11px] text-gray-300 truncate">{m.tmdb_title_original}</p>
						{/if}
					</div>
					{#if m.status === 'do_przegrania'}
						<span class="shrink-0 text-[10px] px-2 py-0.5 rounded-full font-medium"
							style="background:#f0f9ff; color:#00B0F0">Do Prz.</span>
					{/if}
				</a>
			{/each}
			...
			{:else}
				<p class="py-6 text-center text-[11px] text-gray-200">{movies.length.toLocaleString('pl-PL')} filmów</p>
			{/if}
		</div>
	{/if}
</div>
